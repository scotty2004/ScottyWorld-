...
// Accounts with these emails are made SUPER_ADMIN whenever they sign in,
// register or use Google sign-in. Keep this list short and lowercase.
const ADMIN_EMAILS = ["maposacourage41@gmail.com"];

async function promoteIfAdminEmail(userId: string) {
  try {
    const u = await db.user.findUnique({ where: { id: userId }, select: { email: true, role: true } });
    if (u && u.role !== "SUPER_ADMIN" && ADMIN_EMAILS.includes(u.email.toLowerCase())) {
...
export async function getCurrentUser() {
  ...
  if (user.role !== "SUPER_ADMIN" && ADMIN_EMAILS.includes(user.email.toLowerCase())) {
    try {
